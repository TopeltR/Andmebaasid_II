CREATE VIEW aktiivsed_mitteaktiivsed_rajad AS
SELECT Rada.raja_kood, Rada.nimetus, Rada.pikkus, Raja_seisundi_liik.nimetus AS hetke_seisund
...
CREATE OR REPLACE VIEW rada_kategooria_omamine AS
SELECT Raja_kategooria_omamine.raja_kood, Raja_kategooria.nimetus || ' (' || Raja_kategooria_tyyp.nimetus || ')' AS kategooria
...
CREATE OR REPLACE VIEW koik_radad AS
SELECT Rada.raja_kood, Rada.nimetus, Rada.pikkus, Raja_seisundi_liik.nimetus AS hetke_seisund
FROM Raja_seisundi_liik
INNER JOIN Rada ON Raja_seisundi_liik.raja_seisundi_liik_kood = Rada.raja_seisundi_liik_kood;
...
CREATE OR REPLACE VIEW radade_koondaruanne AS
SELECT Raja_seisundi_liik.raja_seisundi_liik_kood, upper(Raja_seisundi_liik.nimetus) AS seisundi_nimetus, Count(Rada.raja_kood) AS arv
FROM Raja_seisundi_liik
...
CREATE OR REPLACE VIEW radade_detailid AS
SELECT Rada.raja_kood, Rada.nimetus, Rada.pikkus, Raja_seisundi_liik.nimetus AS hetke_seisund, Isik.e_meil AS registreerija_email, (Trim(Isik.eesnimi || ' ' || Isik.perenimi)) AS registreerija_nimi, Rada.reg_aeg
FROM Raja_seisundi_liik
