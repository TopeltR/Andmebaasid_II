...
RETURNS Rada.raja_kood%TYPE AS $$
UPDATE Rada SET raja_seisundi_liik_kood = 2 WHERE raja_kood = p_rada_kood AND raja_seisundi_liik_kood = 1
RETURNING raja_kood;
$$ LANGUAGE sql SECURITY DEFINER
SET search_path = public, pg_temp;
...
RETURNS Rada.raja_kood%TYPE AS $$
UPDATE Rada SET raja_seisundi_liik_kood = 4 WHERE raja_kood = p_rada_kood AND raja_seisundi_liik_kood IN (2,3)
RETURNING raja_kood;
$$ LANGUAGE sql SECURITY DEFINER
SET search_path = public, pg_temp;
...
CREATE OR REPLACE FUNCTION f_muuda_rada(p_vana_raja_kood Rada.raja_kood%TYPE, p_uus_raja_kood Rada.raja_kood%TYPE, p_rada_nimetus Rada.nimetus%TYPE, p_rada_pikkus Rada.pikkus%TYPE)
RETURNS Rada.raja_kood%TYPE AS $$
UPDATE Rada SET raja_kood = p_uus_raja_kood, nimetus = p_rada_nimetus, pikkus = p_rada_pikkus WHERE raja_kood = p_vana_raja_kood AND raja_seisundi_liik_kood IN (1,3)
RETURNING raja_kood;
$$ LANGUAGE sql SECURITY DEFINER
SET search_path = public, pg_temp;
